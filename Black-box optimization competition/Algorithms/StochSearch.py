...
def solveSA(PO, GBest, funcName, iterMax, dim, interval, T_Bound, T, rate):
...
def getNewList(list1, list2, bounds):
    new_list = []
...
#     # print(x[item]) # change to the real formula
#     result = funcName(x[item])
#     z.append(result)
#   return np.array(z)



def hybrid_stochastic(f_, N_x, bounds_t, N=100):

    """Define relevant parameters"""
    
    def f(x):
        f_obj = f_(x)
        return f_obj, 0.1*f_obj
    
    dimensions= N_x
    # redefine bounds as list
    bounds = [[bound_i[0],bound_i[1]] for bound_i in bounds_t]
    #creates bounds [[x1,x2],[x3,x4],[x5,x6]....] 

    PopSize = int(np.sqrt(N)/2)
    MaxIter = N - PopSize

    numRerank  = 2  #How many iterations to reclassify
    numRebound = int(MaxIter/8) #How many iterations to reduce the search space
    ...
    Parameters of PSO
    """
    VBound = [-(bounds[0][1]-bounds[0][0])*0.75, (bounds[1][1]-bounds[1][0])*0.75]
    c1=2.8
    c2=1.3
...
# test1 = hybrid_stochastic(f_d2, N_x, bounds, N=10000)
# N_x = 3
# bounds = [(-2.0, 2.0) for i in range(N_x)]
# test2 = hybrid_stochastic(f_d3, N_x, bounds, N=10000)
