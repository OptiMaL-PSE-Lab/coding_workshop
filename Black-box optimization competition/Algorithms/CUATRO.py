...
def optimizer_dummy(f_, N_x: int, bounds: List[Tuple[float]], N: int = 100) -> (float, List[float]):
# '''
...
# '''
    ...
    sol = CUATRO(f, x0, np.max([(b[1] - b[0])/2 for b in bounds]), 
                    bounds = bounds, N_min_samples = 6, tolerance = 1e-10,\
                    beta_red = 0.9, rnd = 0, method = 'global', \
                    max_f_eval=N, max_iter=N)
    return sol['f_best_so_far'][-1], sol['x_best_so_far'][-1]


N_x = 2
bounds = [(-2.0, 2.0) for i in range(N_x)]
test1 = optimizer_dummy(f_d2, N_x, bounds, N=1000)
N_x = 3
bounds = [(-2.0, 2.0) for i in range(N_x)]
test2 = optimizer_dummy(f_d3, N_x, bounds, N=1000)
